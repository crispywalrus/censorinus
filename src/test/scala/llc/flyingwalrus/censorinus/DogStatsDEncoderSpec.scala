...
class DogStatsDEncoderSpec extends FlatSpec with Matchers {
  ...
  it should "encode events" in {
    ...
    Encoder.encode(m).get should be(
      "_e{6,14}:foobar|derp derp derp|d:%d|h:fart|k:agg_key|p:low|s:user|t:error|#foo:bar".format(now)
    )
  }
